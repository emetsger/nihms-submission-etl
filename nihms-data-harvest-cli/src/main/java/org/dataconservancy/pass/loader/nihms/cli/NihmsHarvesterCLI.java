...
public class NihmsHarvesterCLI 
{
    ...
    @Option(name = "-p", aliases = { "-inprocess", "--inprocess"}, usage = "In Process NIHMS publication status. By default all available CSV data is processed. "
                                                                         + "If one or more status type is specified, only publications matching the selected status(es) will be processed.")
    ...
    @Option(name = "-s", aliases = { "-startDate", "--startDate" }, usage = "DateTime to start the query against NIHMS data. This will cause "
                                                                         + "a return of all records published since the date provided. Syntax must be mm-yyyy. This value will override the "
